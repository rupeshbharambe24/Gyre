...
//! nothing" transport. Run it with `cargo run -p whirl-obfs`.

use whirl_obfs::{default_transports, shannon_entropy_bits_per_byte};
...
fn main() {
    println!("Whirlpool · Addition 1 — obfuscation / pluggable transports");
    println!("{}", "-".repeat(72));

    // A sample of (here, deliberately structured) first-hop bytes.
    let inner = b"GET /whirlpool HTTP/1.1\r\nHost: relay\r\n\r\n<inner encrypted payload>";
    let key = [0x24u8; 32];
    println!(
