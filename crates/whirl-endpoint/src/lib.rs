...
impl Ratchet {
    ...
    pub fn next_message_key(&mut self) -> [u8; 32] {
        let message_key = prf(&self.chain_key, b"whirlpool-ratchet-message");
        let next_chain = prf(&self.chain_key, b"whirlpool-ratchet-chain");
...
pub const UNIFORM_FINGERPRINT: &[u8] = b"whirlpool/1 uniform-client";
...
pub fn naive_fingerprint(user_id: u64) -> Vec<u8> {
    let mut fp = b"whirlpool/1 client=".to_vec();
