...
| ✅ | Memory-hard PoW via pure-Rust `equix`/`hashx` | done, operator-selectable | `gyre-shield::pow`: pluggable `PowAlgorithm` trait, `EquixPow` behind `--features equix` (LGPL isolated); SHA-256 stays the MIT default. Threaded through the live gate **and the daemon** — `gyre-rendezvous --pow equix` runs a memory-hard gate, demonstrated across real processes (a client solves Equi-X and is admitted). Algorithm bound into the challenge tag, fail-closed on unsupported (refuses `--pow equix` loudly if not built with the feature). Adversarially reviewed (0 findings) |
